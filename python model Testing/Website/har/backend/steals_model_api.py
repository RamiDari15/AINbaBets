...
def prepare_input_features(player_stats, team_stats, opponent_stats, line):
    ...
    return np.array([
        ...
        averages.get('steals', 0),
        averages.get('minutes', 0),
        own_stats.get("steals", 0),
...
def get_model_prediction(player_id, team_id, opponent_team_id, season_year, line):
    ...
    model = NBAStealModel(input_dim)
    model.load_state_dict(torch.load("best_steal_model.pt"))
    model.eval()
...
def main():
    if len(sys.argv) < 6:
        print("Usage: steals_model.py <player_id> <team_id> <opponent_team_id> <season_year> <line>")
        sys.exit(1)
